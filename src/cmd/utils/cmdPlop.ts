...
import { selectGenerator } from '../../prompts/selectGenerator';
...
export const cmdPlop = async ({args : [ genName, ...newArgs], configPath, force = false, dest = process.cwd() }: CmdPlopParams ): Promise<boolean> => {
    ...
    const generatorNotInList= !generatorList.map(({name}) => name).includes(generatorName)
    ...
    if(!generatorName || generatorName && generatorNotInList){
        ...
        if(generatorName && generatorNotInList){
            logger(chalk.yellow.bold('⚠'), chalk.yellow('Generator'), chalk.cyanBright(generatorName), chalk.yellow("isn't in the list"));
            message = 'Please choose a valid generator'
        }
        generatorName = await selectGenerator({message, generatorList})
    } else {
        logger(chalk.green.bold('✦'), chalk.bold('You use generator'), chalk.cyanBright(generatorName)); 
    }
