...
vi.mock("~/src/utils/logger", () => ({
	logger: (...args: any[]) => MockLogger(...args),
	logError: vi.fn(),
}));
...
vi.mock("../filePlop", () => ({
	deepFilePlop: async () => MockDeepFilePlop(true),
}));
...
describe("constructorPlop", () => {
	...
	it("should handle FILE_PLOP and call deepFilePlop with correct args", async () => {
		...
		expect(MockDeepFilePlop).toHaveBeenCalledWith(true);
	});
