...
import type { Config } from "~/src/test/plop/Config";
import constructorNodePlop from "~/src/test/plop/plopfile";
...
describe("defaultInit", () => {
	...
	it("should generate prompts and run actions with correct config", async () => {
		...
		const { stdout, exitCode } = await renderCommand({
			...
			setup: async ({ program, processTerm }) => {
				program.command("init").action(async () => {
					...
					const typePrompt = (prompts as any).find(
						(p: any) => p.name === "type",
					);
					...
					expect(typePrompt?.choices({ ext: "json" })).toEqual([
						"camelCase",
						"snakeCase",
					]);
		...
		expect(stdout).toContainLine("✔ [copyTo] config.cli");
		expect(stdout).toContainLine("✔ [addFolder] .cli");
